...
class TestImgNestSpatial(unittest.TestCase):
    def test_intersect(self):
        with tests.temp_dir() as base_dir:
            ...
            for zoom, image_names in expected:
                ...
                self.assertEqual(image_names, fnames)
...
class TestImgNest(unittest.TestCase):
    def test_nest(self):
        ...
        self.assertTrue(z0_key in nest_z0_z1._ancestry.keys())
        self.assertEqual(len(nest_z0_z1._ancestry), 1)

        # check that it has figured out that all the z1 images are children of
        # the only z0 image
        for img in z1.images:
            key = ('aerial z0 test', z0.images[0])
            self.assertTrue('aerial z1 test', img) in nest_z0_z1._ancestry[key]

        x1_y0_z1, = [img for img in z1.images
                     if img.filename.endswith('z_1/x_1_y_0.png')]

        self.assertEqual((1, 0, 1), _tile_from_img(x1_y0_z1))

        self.assertEqual([(2, 0, 2), (2, 1, 2), (3, 0, 2), (3, 1, 2)],
        ...
        for name in nest_z0_z1._collections_by_name.keys():
            for img in nest_z0_z1._collections_by_name[name].images:
                ...
                self.assertTrue(img in collection.images)

        self.assertEqual(nest_z0_z1._ancestry, nest_from_config._ancestry)
        ...
        self.assertEqual(nest_z0_z1._ancestry,
